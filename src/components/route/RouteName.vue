<template>
  <div >
    <ModalPopup :yesNoOption="true" v-if="showRemoveRoutePopup" @yesAnswer="confirmRemoveRoute(true)" @noAnswer="confirmRemoveRoute(false)">
      <div>Are you sure that you want to remove this route?</div>
    </ModalPopup>
    <ModalPopup :yesNoOption="true" v-if="showArchiveRoutePopup" @yesAnswer="confirmArchiveRoute(true)" @noAnswer="confirmArchiveRoute(false)">
      <div>You are about to archive this route.<br>You will not be able to make any changes until you restore it.<br>Do you want to continue?</div>
    </ModalPopup>
    <ClickToEdit
      :value="route.name"
      ...
      :active="route.active"
      :class="route.active ? 'font-bold' : 'font-normal'"
      class="route-name"
    />
    ...
    <template #content>
        <div class="tooltip-popup">This will set this route as the active route to modify it</div>
      </template>
    </Popper>
    <Popper :interactive="false" :hover="true">
    <button
    ...
    <template #content>
        <div class="tooltip-popup">This will calculate the shortest route based on the selected stops</div>
      </template>
    </Popper>
    <Popper :interactive="false" :hover="true">
    <button
    ...
    <template #content>
        <div class="tooltip-popup">This will zoom the map to show the complete route</div>
      </template>
    </Popper>
    <Popper :interactive="false" :hover="true">
    <button
    ...
    <button
      ...
      class="bg-red fg-white clickable"
    >
      Remove
    </button>
    <template #content>
  ...
  methods: {
    ...
    zoomToActiveRoute(){
      ...
      if (this.$store.state.activeRoute.searchPredefined){
        ...
        highestLon = this.$store.state.activeRoute.stops[0].coordinate.longitude
        stopStartIdx = 1
      }else{
      ...
      for (let i=stopStartIdx;i<this.$store.state.activeRoute.stops.length;i++){
        if (this.$store.state.activeRoute.stops[i].coordinate.latitude < lowestLat){
          lowestLat = this.$store.state.activeRoute.stops[i].coordinate.latitude
        }
        if (this.$store.state.activeRoute.stops[i].coordinate.latitude > highestLat){
          highestLat = this.$store.state.activeRoute.stops[i].coordinate.latitude
        }
        if (this.$store.state.activeRoute.stops[i].coordinate.longitude < lowestLon){
          lowestLon = this.$store.state.activeRoute.stops[i].coordinate.longitude
        }
        if (this.$store.state.activeRoute.stops[i].coordinate.longitude > highestLon){
          highestLon = this.$store.state.activeRoute.stops[i].coordinate.longitude
        }
      }
      ...
      this.$store.commit("setNewMapRegion",{centerLat:centerLat,centerLon:centerLon,diffLat:diffLat,diffLon:diffLon})
    },
