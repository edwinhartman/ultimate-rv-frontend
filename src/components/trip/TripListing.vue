<template>
  <div class="text-left pl-2">
    <TripOverview
      v-for="route in activeTrips"
      :key="route._id"
      :route_prop="route"
    />
      
    <div v-if="archivedTrips.length > 0 && $store.state.showArchivedTrips" class="archived-routes-div">
      <div class="font-bold relative">Archived Trips <img v-if="!showArchivedTrips" src="/static/arrow-down-icon.png" class="icon min-max-button2 clickable" @click="toggleShowArchivedTrips()"><img v-if="showArchivedTrips" src="/static/arrow-up-icon.png" class="icon min-max-button2 clickable" @click="toggleShowArchivedTrips()"></div>
      <div v-if="showArchivedTrips">
    <ArchivedTrips v-for="route in archivedTrips" :key="route._id" :route_prop="route" />
      </div>
    </div>
    <div v-if="adminToken != null && $store.state.showSystemTrips && !$store.state.hideAdminFunctions" class="system-routes-div">
      <div class="font-bold relative">System Trips <img v-if="!showSystemTrips" src="/static/arrow-down-icon.png" class="icon min-max-button2 clickable" @click="toggleShowSystemTrips()"><img v-if="showSystemTrips" src="/static/arrow-up-icon.png" class="icon min-max-button2 clickable" @click="toggleShowSystemTrips()"></div>
