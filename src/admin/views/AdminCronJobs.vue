<template>
    <div class="main_div">
        ...
        <table v-if="jobs.length > 0">
            ...
            <tr v-for="(job,idx) in jobs" :key="job._id" @click="editExistingJob(idx)" :class="job.enabled?'active-job':'inactive-job'">
                <td>{{ job.title }}</td>
                <td>{{ job.task.name }}</td>
                <td>{{ job.cronTime }}</td>
                <td>{{ job.enabled }}</td>
        ...
        <div v-if="addEditFormVisible" class="add-edit-form">
            ...
            <div>Task Name: <select name="" id="" v-model="selectedTask">
                <option v-for="task in tasks" :key="task._id" :value="task._id">{{ task.name }}</option>
                </select></div>
            <div>Enabled: <input type="checkbox" name="" id="" v-model="enabled"></div>
            <div>Time: {{ jobTime.cronTime }}
    ...
    methods:{
        ...
        saveJob(){
            ...
            axios({
                ...
                this.jobs = res.data.jobs
                axios({
                    url:process.env.VUE_APP_BACKEND_CONNECTION_URI + "/admin/resetBackgroundJobs",
                    method:'get'
                }).then((res)=>{ })
            })
        },
